...
const stripe = new Stripe("sk_test_51ODoJFAQYHZn8ah9WDYZSBSjs4pRWQshcZfYhaSBJNQnVzi6kbDisu9wIqlrdbmcTOmmG95HHujZ1PvEYLp6ORhe00K0D8eLz5");
...
exports.messageseen = async(req, res) => {
    ...
    try {
        ...
        db.query(
            "SELECT * FROM chatmessages WHERE user_id = ? AND to_id = ? ORDER BY id desc", [to_id, user_id],
            (selectErr, selectResults) => {
                if (selectErr) {
                ...
                db.query(
                    "UPDATE chatmessages SET `read` = 'Yes' WHERE id = ? AND to_id = ?", [idd, user_id],
                    (updateErr) => {
                        if (updateErr) {
...
exports.paymentdatasave = async (req, res) => {
    ...
    try {
      ...
      const start_date = moment.tz(new Date(), 'Europe/Oslo').format('YYYY-MM-DD HH:mm:ss');
      const end_date = moment.tz(new Date(), 'Europe/Oslo').add(dayss, 'days').format('YYYY-MM-DD HH:mm:ss');
      const date = moment.tz(new Date(), 'Europe/Oslo').format('YYYY-MM-DD HH:mm:ss');
      ...
      await new Promise((resolve, reject) => {
        db.query(insertQuery, [user_id, start_date, end_date, dayss, paymentdetail.plan,paymentdetail.amount, detail.id, paymentdetail.currency, detail.livemode, detail.status, date], (err, result) => {
          if (err) {
      ...
      await new Promise((resolve, reject) => {
        db.query(updateQuery, [start_date, end_date, dayss, paymentdetail.plan,paymentdetail.amount, detail.id, paymentdetail.currency, detail.livemode, detail.status, date, user_id], (err, result) => {
          if (err) {
      ...
      res.status(200).json({ message: 'Membership data saved and updated successfully' });
